...
{{- define "harbor.redis.username" -}}
  {{- with .Values.redis }}
    {{- ternary "" (.external.username | urlquery) (eq .type "internal") }}
  {{- end }}
{{- end -}}
...
{{- define "harbor.redis.external.password" -}}
  {{- if .Values.redis.external.existingSecret }}
    ...
    {{- $password | urlquery -}}
  {{- else }}
    {{- .Values.redis.external.password | urlquery }}
  {{- end -}}
{{- end -}}
...
{{- define "harbor.redis.cred" -}}
    ...
    {{- ternary "" (printf "%s:%s@" $username $password) (and (not $username) (not $password)) }}
{{- end -}}
